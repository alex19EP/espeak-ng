...
set(PHONEME_TMP_DIR ${CMAKE_CURRENT_BINARY_DIR}/phsource)
set(DICT_TMP_DIR ${CMAKE_CURRENT_BINARY_DIR}/dictsource)
...
file(CREATE_LINK "${PHONEME_SRC_DIR}" "${PHONEME_TMP_DIR}" SYMBOLIC)

add_custom_command(
  OUTPUT "${DATA_DIST_DIR}/intonations"
  COMMAND env ESPEAK_DATA_PATH="${DATA_DIST_ROOT}" "$<TARGET_FILE:espeak-ng>" --compile-intonations
  WORKING_DIRECTORY "${PHONEME_SRC_DIR}"
  COMMENT "Compile intonations"
  DEPENDS "$<TARGET_FILE:espeak-ng>"
)
...
add_custom_command(
  ...
  COMMAND env ESPEAK_DATA_PATH="${DATA_DIST_ROOT}" "$<TARGET_FILE:espeak-ng>" --compile-phonemes
  WORKING_DIRECTORY "${PHONEME_SRC_DIR}"
...
foreach(_dict_name ${_dict_compile_list})
  ...
  add_custom_command(
    ...
    COMMAND env ESPEAK_DATA_PATH="${DATA_DIST_ROOT}" "$<TARGET_FILE:espeak-ng>" --compile=${_dict_name}
...
if (HAVE_MBROLA AND USE_MBROLA)
  ...
  foreach(_mbl ${_mbrola_lang_list})
    ...
    add_custom_command(
      OUTPUT "${_mbl_out}"
      COMMAND env ESPEAK_DATA_PATH="${DATA_DIST_ROOT}" "$<TARGET_FILE:espeak-ng>" --compile-mbrola="${_mbl_src}"
      DEPENDS "$<TARGET_FILE:espeak-ng>" "${_mbl_src}"
    )
  endforeach(_mbl)
endif()
